                      ...
                      argon2
                          ...
                          .then(hash2 => {//hash #1
                              ...
                              fetch("https://api.jamiez.co.uk/pwdmanager/login", {
                                  ...
                                  if(res.data=="Invalid parameters" || res.data=="Email Not Registered" || res.data=="Invalid password"){////////////NEEDS IF WEBAUTHN
                                      ...
                                      if(data.verified==0){//webauthn required
                                          ...
                                          ////////////////////////////////////////////////////ENCRYPT PASSWORD
                                          fetch('https://api.jamiez.co.uk/pwdmanager/webauthn/request-keys', {
                                              method: 'POST',
                                              headers: {
                                                'content-type': 'Application/Json'
                                              },
                                            ...
                                            var jsonObj = JSON.parse(keys.data);
                                            if(jsonObj.length==0){
                                              //no keys on the account
                                              console.log("No keys");
                                            } else if (jsonObj.length==1){
                                              //If there is only one key we dont need a list to select from
                                              fetch('https://api.jamiez.co.uk/pwdmanager/webauthn/login', {
                                                  method: 'POST',
                                                  headers: {
                                                    'content-type': 'Application/Json'
                                                  },
                                                  ...
                                                  result.text().then(data => ({
                                                    data: data
                                                      ...
                                                      const credentials = await (0, _client.solveLoginChallenge)(JSON.parse(challenge));
                                                      fetch('https://api.jamiez.co.uk/pwdmanager/webauthn/login-challenge', {
                                                        method: 'POST',
                                                        headers: {
                                                          'content-type': 'Application/Json'
                                                        },
                                                        ...
                                                        result.text().then(data => ({
                                                          data: data
                                                            ...
                                                            if (loggedIn.data) {
                                                              displayMessage('You are logged in');//send to dashboard
                                                              return;
                                                            }
                                                            displayMessage('Invalid credential');
                                                          })
